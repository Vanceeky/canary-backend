...
- **Real push delivery (FCM) is implemented but not yet live-verified**
  against a real Firebase project/device. Falls back to
  `ConsoleNotificationService` (logs exactly what would be sent) whenever
  `FIREBASE_SERVICE_ACCOUNT_JSON_BASE64` isn't configured.
...
- **Exactly one notification per event, chosen by a fixed priority order**
  ...
  request, reversing the original design).
